...
def analyse_file_name(fname):
    ...
    re_year = re.findall('\((.*?)-', coords) # This will extract the file year from the original file name
    ...
    fname2 = os.path.join(os.path.join(os.path.join(path, outputdir),str(dirs[-1])), str(dirs[-1]) + '_' + str(*re_year) + '_' + str(*re_num) + '_' + coordsFname + coordsExt[1]) # This is the new filename
    print(fname)
    print(fname2)
